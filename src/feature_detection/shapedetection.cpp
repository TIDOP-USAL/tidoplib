...
void GeneralizedHough::accumulate(cv::Mat &input_img)
 {
  ...
  float pi_half = I3D_PI * 0.5f;
	...
	for (int j=0; j<nl; ++j) {
        ...
        int angleindex = (int)((phi + I3D_PI*0.5f)*inv_deltaphi); // index associated with angle (0 index = -90 degrees)
  ...
  if ( phimax == I3D_PI && phimin == -I3D_PI ) R--;
  ...
  cv::Mat template_img = cv::imread(templateimg, 1);
   if ( original_img.empty() ) return; //... devolver error
	...
	for (int j=0; j<nl; ++j) {
        ...
        rpt.phi = ((a > 0) ? a - I3D_PI / 2 : a + I3D_PI / 2);
...
void GeneralizedHough::readRtable()
{
	...
	float range = I3D_PI/intervals;
	for (std::vector<Rpoint>::size_type t = 0; t < pts.size(); ++t){
    int angleindex = (int)((pts[t].phi + I3D_PI / 2) / range);
