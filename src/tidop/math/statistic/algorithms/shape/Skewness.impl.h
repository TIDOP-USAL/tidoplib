...
public:
    ...
    auto eval(const DescriptiveStatistics<T> &stat) const -> double override
    {
        ...
        double skew = stat.centralMoments().thirdMoment();
        ...
        if(stat.isSample()) {
            ...
            return skew * n / ((n - 1) * (n - 2) * _variance * stat.standardDeviation());
        } else {
            return skew / (n * _variance * stat.standardDeviation());
        }
    }
...
public:
    ...
    auto eval(const DescriptiveStatistics<T> &stat) const -> double override
    {
        return (stat.mean() - stat.mode()) / stat.standardDeviation();
    }
