...
public:
    ...
    Scaling() : scale(Vector<T, Dim>::unit()) {}
    ...
    Scaling(T sx, T sy) 
    {
        static_assert(Dim == 2, "Constructor for 2D Scaling. Use the 3D Scaling constructor: Scaling(T sx, T sy, T sz).");
    ...
    {
        static_assert(Dim == 2, "Constructor for 3D Scaling. Use the 2D Scaling constructor: Scaling(T sx, T sy).");
    ...
    Scaling(Scaling &&scaling) TL_NOEXCEPT
      : scale(std::forward<Vector<T, Dim>>(scaling))
    {
    }
    ...
    {
        if (this != &scaling) {
            this->scale = std::forward<Vector<T, Dim>>(scaling.scale);
        }
    ...
    auto z() -> T
    {
        static_assert(Dim == 3, "Method not valid for 2D Scaling");
        return this->scale[2];
    }
    ...
    {
        return Scaling(consts::one<T>/this->scale);
    }
    ...
    {
        TL_ASSERT(Dim == vector.size(), "Invalid Vector dimensions");
...
public:
    ...
    Translation() : translation(Vector<T, Dim>::zero()) {}
   
    Translation(T tx, T ty) 

    {
        static_assert(Dim == 2, "Constructor for 2D Translation. Use the 3D Translation constructor: Translation(T tx, T ty, T tz).");
    ...
    {
        static_assert(Dim == 3, "Constructor for 3D Translation. Use the 2D Translation constructor: Translation(T tx, T ty).");
    ...
    Translation(Translation &&translation) TL_NOEXCEPT
      : translation(std::forward<Vector<T, Dim>>(translation))
    {
    }
    ...
    {
        if (this != &translate) {
            this->translation = std::forward<Vector<T, Dim>>(translate.translation);
        }
    ...
    auto z() -> T
    {
        static_assert(Dim == 3, "Method not valid for 2D translations");
