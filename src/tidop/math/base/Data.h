...
public:
     ...
     */
    constexpr explicit Data(const T *data, size_t size = 0);
     ...
     */
    constexpr auto operator = (const Data &matrix) -> Data &;
     ...
     */
    constexpr auto operator = (Data &&matrix) noexcept -> Data &;
     ...
     */
    constexpr auto operator=(T value) -> Data&;
    ...
    // */
    //[[nodiscard]]
    //constexpr auto operator < (const Data &data) const -> bool;
     ...
     */
    [[nodiscard]]
    constexpr auto size() const noexcept -> size_t { return mData.size(); }
...
public:
     ...
     */
    constexpr Data(size_t size, T val);
     ...
     */
    constexpr Data(const T *data, size_t size);
...
template<typename T, size_t _size>
constexpr Data<T, _size>::Data(size_t  /*size*/)
  : mData()
{
    this->mData.fill(-std::numeric_limits<T>().max());
}
...
template<typename T, size_t _size>
constexpr Data<T, _size>::Data(size_t  /*size*/, T val)
  : mData()
{
    this->mData.fill(val);
}
...
template<typename T, size_t _size>
constexpr Data<T, _size>::Data(const Data &matrix)
  : mData(matrix.mData)
{
}

template<typename T, size_t _size>
constexpr Data<T, _size>::Data(Data &&matrix) noexcept
  : mData(std::move(matrix.mData))
{
}

template<typename T, size_t _size>
constexpr Data<T, _size>::Data(std::initializer_list<T> values)
{
...
template<typename T, size_t _size>
constexpr  Data<T, _size>::Data(const T *data,
                                size_t /*size*/)
  : mData(data)
{
}
...
template<typename T, size_t _size>
constexpr auto Data<T, _size>::operator = (const Data &matrix) -> Data &
{
    if (this != &matrix) {
...
template<typename T, size_t _size>
constexpr auto Data<T, _size>::operator = (Data &&matrix) noexcept -> Data &
{
    if (this != &matrix) {
...
template<typename T, size_t _size>
constexpr auto Data<T, _size>::at(size_t position) -> reference
{
    return mData.at(position);
}

template<typename T, size_t _size>
constexpr auto Data<T, _size>::at(size_t position) const -> const_reference
{
    return mData.at(position);
}

template<typename T, size_t _size>
constexpr auto Data<T, _size>::operator[](size_t position) noexcept -> reference
{
    return mData[position];
}

template<typename T, size_t _size>
constexpr auto Data<T, _size>::operator[](size_t position) const noexcept -> const_reference
{
    return mData[position];
}

template<typename T, size_t _size>
constexpr auto Data<T, _size>::operator=(T value) -> Data &
{
    mData.fill(value);
    return *this;
}
...
//template<typename T, size_t _size>
//constexpr auto Data<T, _size>::operator == (const Data &data) const -> bool
//{
...
template<typename T, size_t _size>
constexpr auto Data<T, _size>::front() noexcept -> reference
{
    return mData.front();
}

template<typename T, size_t _size>
constexpr auto Data<T, _size>::front() const noexcept -> const_reference
{
    return mData.front();
}

template<typename T, size_t _size>
constexpr auto Data<T, _size>::back() noexcept -> reference
{
    return mData.back();
}

template<typename T, size_t _size>
constexpr auto Data<T, _size>::back() const noexcept -> const_reference
{
    return mData.back();
}

template<typename T, size_t _size>
constexpr auto Data<T, _size>::begin() noexcept -> iterator
{
    return mData.begin();
}

template<typename T, size_t _size>
constexpr auto Data<T, _size>::begin() const noexcept -> const_iterator
{
    return mData.begin();
}

template<typename T, size_t _size>
constexpr auto Data<T, _size>::end() noexcept -> iterator
{
    return mData.end();
}

template<typename T, size_t _size>
constexpr auto Data<T, _size>::end() const noexcept -> const_iterator
{
    return mData.end();
}

template<typename T, size_t _size>
constexpr auto Data<T, _size>::data() noexcept -> pointer
{
    return mData.data();
}

template<typename T, size_t _size>
constexpr auto Data<T, _size>::data() const noexcept -> const_pointer
{
    return mData.data();
}
...
template<typename T>
constexpr Data<T, DynamicData>::Data(size_t size)
  : mData(size, -std::numeric_limits<T>().max())
{
}

template<typename T>
constexpr Data<T, DynamicData>::Data(size_t size, T val)
  : mData(size, val)
{
}

template<typename T>
constexpr Data<T, DynamicData>::Data(const Data &matrix)
  : mData(matrix.mData)
{
}

template<typename T>
constexpr Data<T, DynamicData>::Data(Data &&matrix) noexcept
  : mData(std::move(matrix.mData))
{
}

template<typename T>
constexpr Data<T, DynamicData>::Data(std::initializer_list<T> values)
{
...
template<typename T>
constexpr Data<T, DynamicData>::Data(const T *data, size_t size)
    : mData(size, -std::numeric_limits<T>().max())
{
...
template<typename T>
constexpr auto Data<T, DynamicData>::operator = (const Data<T, DynamicData> &matrix) -> Data &
{
    if (this != &matrix) {
        this->mData = matrix.mData;
    }
...
template<typename T>
constexpr auto Data<T, DynamicData>::operator = (Data<T, DynamicData> &&matrix) noexcept -> Data &
{
    if (this != &matrix) {
        this->mData = std::move(matrix.mData);
    }
...
template<typename T>
constexpr auto Data<T, DynamicData>::at(size_t position) -> reference
{
    return mData.at(position);
}

template<typename T>
constexpr auto Data<T, DynamicData>::at(size_t position) const -> const_reference
{
    return mData.at(position);
}

template<typename T>
constexpr auto Data<T, DynamicData>::operator[](size_t position) noexcept -> reference
{
    return mData[position];
}

template<typename T>
constexpr auto Data<T, DynamicData>::operator[](size_t position) const noexcept -> const_reference
{
    return mData[position];
}

template<typename T>
constexpr auto Data<T, DynamicData>::operator=(T value) -> Data&
{
    std::fill(mData.begin(), mData.end(), value);
    return *this;
}
...
template<typename T>
constexpr auto Data<T, DynamicData>::front() noexcept -> reference
{
    return mData.front();
}

template<typename T>
constexpr auto Data<T, DynamicData>::front() const noexcept -> const_reference
{
    return mData.front();
}

template<typename T>
constexpr auto Data<T, DynamicData>::back() noexcept -> reference
{
    return mData.back();
}

template<typename T>
constexpr auto Data<T, DynamicData>::back() const noexcept -> const_reference
{
    return mData.back();
}

template<typename T>
constexpr auto Data<T, DynamicData>::begin() noexcept  -> iterator
{
    return mData.begin();
}

template<typename T>
constexpr auto Data<T, DynamicData>::begin() const noexcept-> const_iterator
{
    return mData.begin();
}

template<typename T>
constexpr auto Data<T, DynamicData>::end() noexcept -> iterator
{
    return mData.end();
}

template<typename T>
constexpr auto Data<T, DynamicData>::end() const noexcept -> const_iterator
{
    return mData.end();
}

template<typename T>
constexpr auto Data<T, DynamicData>::data() noexcept -> pointer
{
    return mData.data();
}

template<typename T>
constexpr auto Data<T, DynamicData>::data() const noexcept -> const_pointer
{
    return mData.data();
}
...
[[nodiscard]] 
constexpr auto operator==(const Data<T, Size1> &lhs, const Data<T, Size2> &rhs) -> bool
{
    if constexpr (Size1 != DynamicData && Size2 != DynamicData) {
        if constexpr (Size1 != Size2) return false;
    }
...
//    return simd::equal(lhs.data(), rhs.data(), lhs.size());
//#else 
//    return std::equal(lhs.begin(), lhs.end(), rhs.begin());
//#endif
}
