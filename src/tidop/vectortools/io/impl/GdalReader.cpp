...
auto convertPoint(const OGRPoint *ogrPoint) -> std::unique_ptr<GPoint>
{
    return std::make_unique<GPoint>(ogrPoint->getX(),
                                    ogrPoint->getY());
}

auto convertPoint3D(const OGRPoint *ogrPoint) -> std::unique_ptr<GPoint3D>
{
...
auto convertLineString(const OGRLineString *ogrLineString) -> std::unique_ptr<GLineString>
{
...
auto convertLineString3D(const OGRLineString *ogrLineString) -> std::unique_ptr<GLineString3D>
{
    size_t size = static_cast<size_t>(ogrLineString->getNumPoints());
...
auto convertPolygon(OGRPolygon *ogrPolygon) -> std::unique_ptr<GPolygon>
{
...
auto convertPolygon3D(OGRPolygon *ogrPolygon) -> std::unique_ptr<GPolygon3D>
{
    const OGRLinearRing *ogr_linear_ring = ogrPolygon->getExteriorRing();
    size_t size = ogr_linear_ring->getNumPoints();
...
auto convertMultiPoint(OGRMultiPoint *ogrMultiPoint) -> std::unique_ptr<GMultiPoint>
{
...
auto convertMultiPoint3D(OGRMultiPoint *ogrMultiPoint) -> std::unique_ptr<GMultiPoint3D>
{
    auto size = static_cast<size_t>(ogrMultiPoint->getNumGeometries());
...
auto convertMultiLineString(OGRMultiLineString *ogrMultiLineString) -> std::unique_ptr<GMultiLineString>
{
...
auto convertMultiLineString3D(OGRMultiLineString *ogrMultiLineString) -> std::unique_ptr<GMultiLineString3D>
{
    size_t size = static_cast<size_t>(ogrMultiLineString->getNumGeometries());
...
auto convertMultiPolygon(OGRMultiPolygon *ogrMultiPolygon) -> std::unique_ptr<GMultiPolygon>
{
...
auto convertMultiPolygon3D(OGRMultiPolygon *ogrMultiPolygon) -> std::unique_ptr<GMultiPolygon3D>
{
    size_t size = static_cast<size_t>(ogrMultiPolygon->getNumGeometries());
...
auto convertEntity(OGRGeometry *ogrGeometry) -> std::unique_ptr<GraphicEntity>
{
    ...
    case wkbUnknown:
        break;
    case wkbPoint:
        graphic_entity = convertPoint(dynamic_cast<OGRPoint *>(ogrGeometry));
        break;
    case wkbLineString:
        graphic_entity = convertLineString(dynamic_cast<OGRLineString *>(ogrGeometry));
        break;
    case wkbPolygon:
        graphic_entity = convertPolygon(dynamic_cast<OGRPolygon *>(ogrGeometry));
        break;
    case wkbMultiPoint:
        graphic_entity = convertMultiPoint(dynamic_cast<OGRMultiPoint *>(ogrGeometry));
        break;
    case wkbMultiLineString:
        graphic_entity = convertMultiLineString(dynamic_cast<OGRMultiLineString *>(ogrGeometry));
        break;
    case wkbMultiPolygon:
        graphic_entity = convertMultiPolygon(dynamic_cast<OGRMultiPolygon *>(ogrGeometry));
        break;
    ...
    case wkbPoint25D:
        if (dim == 2)
            ...
            graphic_entity = convertPoint3D(dynamic_cast<OGRPoint *>(ogrGeometry));
        break;
    case wkbLineString25D:
        if (dim == 2)
            graphic_entity = convertLineString(dynamic_cast<OGRLineString *>(ogrGeometry));
        else
            graphic_entity = convertLineString3D(dynamic_cast<OGRLineString *>(ogrGeometry));
        break;
    case wkbPolygon25D:
        ...
        if (dim == 2)
            ...
            graphic_entity = convertPolygon3D(dynamic_cast<OGRPolygon *>(ogrGeometry));
        break;
    case wkbMultiPoint25D:
        if (dim == 2)
            graphic_entity = convertMultiPoint(dynamic_cast<OGRMultiPoint *>(ogrGeometry));
        else
            graphic_entity = convertMultiPoint3D(dynamic_cast<OGRMultiPoint *>(ogrGeometry));
        break;
    case wkbMultiLineString25D:
        if (dim == 2)
            graphic_entity = convertMultiLineString(dynamic_cast<OGRMultiLineString *>(ogrGeometry));
        else
            graphic_entity = convertMultiLineString3D(dynamic_cast<OGRMultiLineString *>(ogrGeometry));
        break;
    case wkbMultiPolygon25D:
        if (dim == 2)
            graphic_entity = convertMultiPolygon(dynamic_cast<OGRMultiPolygon *>(ogrGeometry));
        else
            graphic_entity = convertMultiPolygon3D(dynamic_cast<OGRMultiPolygon *>(ogrGeometry));
        break;
...
void convertPenColor(OGRStylePen *ogrStylePen, Pen *pen)
{
...
void convertPenCap(OGRStylePen *ogrStylePen, Pen *pen)
{
    GBool bDefault = false;
...
void convertPenPattern(OGRStylePen *ogrStylePen, Pen *pen)
{
    GBool bDefault = false;
...
void convertPenJoin(OGRStylePen *ogrStylePen, Pen *pen)
{
    GBool bDefault = false;
...
void convertPenName(OGRStylePen *ogrStylePen, Pen *pen)
{
    GBool bDefault = false;
...
void convertPenWidth(OGRStylePen *ogrStylePen, Pen *pen)
{
    GBool bDefault = false;
...
void convertPenPerpendicularOffset(OGRStylePen *ogrStylePen, Pen *pen)
{
    GBool bDefault = false;
...
auto convertStylePen(OGRStylePen *ogrStylePen) -> Pen
{
    Pen pen;

    convertPenColor(ogrStylePen, &pen);
    convertPenCap(ogrStylePen, &pen);
    convertPenPattern(ogrStylePen, &pen);
    convertPenJoin(ogrStylePen, &pen);
    convertPenName(ogrStylePen, &pen);
    convertPenWidth(ogrStylePen, &pen);
    convertPenPerpendicularOffset(ogrStylePen, &pen);
    //readPenPriorityLevel(ogrStylePen, &pen);

    return pen;
}

void convertBrushBackColor(OGRStyleBrush *ogrStyleBrush, Brush &brush)
{
    GBool bDefault = false;
...
void convertBrushForeColor(OGRStyleBrush *ogrStyleBrush, Brush &brush)
{
    GBool bDefault = false;
...
auto convertBrushStyle(OGRStyleBrush *ogrStyleBrush) -> std::pair<Brush::Style, BrushPattern::HatchType>
{
...
auto convertBrushAngle(OGRStyleBrush *ogrStyleBrush) -> std::optional<double>
{
    GBool bDefault = false;
...
auto convertBrushScalingFactor(OGRStyleBrush *ogrStyleBrush) -> std::optional<double>
{
    GBool bDefault = false;
...
auto convertBrushSpacing(OGRStyleBrush *ogrStyleBrush) -> std::optional<Vector2d>
{
    GBool bDefault = false;
...
void convertBrushPattern(OGRStyleBrush *ogrStyleBrush, BrushPattern &pattern)
{
    auto angle = convertBrushAngle(ogrStyleBrush);
    if (angle.has_value()) {
        pattern.setAngle(angle.value());
    }

    auto scaling_factor = convertBrushScalingFactor(ogrStyleBrush);
    if (scaling_factor.has_value()) {
        pattern.setScalingFactor(scaling_factor.value());
    }

    auto spacing = convertBrushSpacing(ogrStyleBrush);
    if (spacing.has_value()) {
        pattern.setSpacing(spacing.value());
    }
}

auto convertStyleBrush(OGRStyleBrush *ogrStyleBrush) -> Brush
{
    Brush brush;

    convertBrushBackColor(ogrStyleBrush, brush);
    convertBrushForeColor(ogrStyleBrush, brush);

    auto [style, hatch] = convertBrushStyle(ogrStyleBrush);
    brush.setStyle(style);

    if (style == Brush::Style::hatch) {
        BrushPattern pattern(hatch);
        convertBrushPattern(ogrStyleBrush, pattern);
...
void convertSymbolAngle(OGRStyleSymbol *ogrStyleSymbol, Symbol *symbol)
{
    GBool bDefault = false;
...
void convertSymbolColor(OGRStyleSymbol *ogrStyleSymbol, Symbol *symbol)
{
    GBool bDefault = false;
...
void convertSymbolName(OGRStyleSymbol *ogrStyleSymbol, Symbol *symbol)
{
    GBool bDefault = false;
...
void convertSymbolOffset(OGRStyleSymbol *ogrStyleSymbol, Symbol *symbol)
{
    GBool bDefault = false;
...
void convertSymbolOutlineColor(OGRStyleSymbol *ogrStyleSymbol, Symbol *symbol)
{
    GBool bDefault = false;
...
void convertSymbolScalingFactor(OGRStyleSymbol *ogrStyleSymbol, Symbol *symbol)
{
    GBool bDefault = false;
...
auto convertStyleSymbol(OGRStyleSymbol *ogrStyleSymbol) -> Symbol
{
    Symbol symbol;

    convertSymbolAngle(ogrStyleSymbol, &symbol);
    convertSymbolColor(ogrStyleSymbol, &symbol);
    convertSymbolName(ogrStyleSymbol, &symbol);
    convertSymbolOffset(ogrStyleSymbol, &symbol);
    convertSymbolOutlineColor(ogrStyleSymbol, &symbol);
    //readSymbolPriorityLevel(ogrStyleSymbol, &symbol);
    convertSymbolScalingFactor(ogrStyleSymbol, &symbol);

    return symbol;
}

void convertLabelAnchorPosition(OGRStyleLabel *ogrStyleLabel, Label *label)
{
    GBool bDefault = false;
...
void convertLabelAngle(OGRStyleLabel *ogrStyleLabel, Label *label)
{
    GBool bDefault = false;
...
void convertLabelBackgroundColor(OGRStyleLabel *ogrStyleLabel, Label *label)
{
    GBool bDefault = false;
...
void convertLabelForegroundColor(OGRStyleLabel *ogrStyleLabel, Label *label)
{
    GBool bDefault = false;
...
void convertLabelOutlineColor(OGRStyleLabel *ogrStyleLabel, Label *label)
{
    GBool bDefault = false;
...
void convertLabelShadowColor(OGRStyleLabel *ogrStyleLabel, Label *label)
{
    GBool bDefault = false;
...
void convertLabelLabelPlacement(OGRStyleLabel *ogrStyleLabel, Label *label)
{
    GBool bDefault = false;
...
void convertLabelOffset(OGRStyleLabel *ogrStyleLabel, Label *label)
{
    GBool bDefault = false;
    GBool bDefault1 = false;
...
void convertLabelStretch(OGRStyleLabel *ogrStyleLabel, Label *label)
{
    GBool bDefault = false;
...
void convertLabelFont(OGRStyleLabel *ogrStyleLabel, Label *label)
{
    GBool bDefault = false;
...
void convertLabelText(OGRStyleLabel *ogrStyleLabel, Label *label)
{
    GBool bDefault = false;
...
auto convertStyleLabel(OGRStyleLabel *ogrStyleLabel) -> Label
{
    Label label;

    convertLabelAnchorPosition(ogrStyleLabel, &label);
    convertLabelAngle(ogrStyleLabel, &label);
    convertLabelBackgroundColor(ogrStyleLabel, &label);
    convertLabelForegroundColor(ogrStyleLabel, &label);
    convertLabelOutlineColor(ogrStyleLabel, &label);
    convertLabelShadowColor(ogrStyleLabel, &label);
    convertLabelLabelPlacement(ogrStyleLabel, &label);
    convertLabelOffset(ogrStyleLabel, &label);
    convertLabelStretch(ogrStyleLabel, &label);
    convertLabelFont(ogrStyleLabel, &label);
    convertLabelText(ogrStyleLabel, &label);

    return label;
}

void convertData(const OGRFeature *ogrFeature,
...
void convertStyles(OGRStyleMgr *ogrStyle, GraphicEntity *gStyle)
{
    ...
    for (int i = 0; i < ogrStyle->GetPartCount(); i++) {
        if ((ogr_style_tool = ogrStyle->GetPart(i)) != nullptr) {
            ...
            case OGRSTCPen:
                gStyle->setPen(convertStylePen(dynamic_cast<OGRStylePen *>(ogr_style_tool)));
                break;
            case OGRSTCBrush:
                gStyle->setBrush(convertStyleBrush(dynamic_cast<OGRStyleBrush *>(ogr_style_tool)));
                break;
            case OGRSTCSymbol:
                gStyle->setSymbol(convertStyleSymbol(dynamic_cast<OGRStyleSymbol *>(ogr_style_tool)));
                break;
            case OGRSTCLabel:
                gStyle->setLabel(convertStyleLabel(dynamic_cast<OGRStyleLabel *>(ogr_style_tool)));
                break;
            case OGRSTCVector:
                break;
            case OGRSTCNone:
...
auto VectorReaderGdal::read(int layerId) const -> std::shared_ptr<GLayer>
{
    ...
    try {
        ...
        layer = this->convertLayer(ogrLayer);
...
auto VectorReaderGdal::read(const std::string &layerName) const -> std::shared_ptr<GLayer>
{
    std::shared_ptr<GLayer> layer;
...
auto VectorReaderGdal::boundingBox() const -> BoundingBox<Point2d>
{
    BoundingBox<Point<double>> bbox;
    for (int i = 0; i < layersCount(); i++) {
        ...
        if (layer->GetExtent(&extent, true) == OGRERR_NONE) {
            BoundingBox<Point2d> layer_box(Point2d(extent.MinX, extent.MinY), Point2d(extent.MaxX, extent.MaxY));
            bbox.extend(layer_box);
        } else {
            auto layer = read(i);
            bbox.extend(layer->boundingBox());
        }
...
auto VectorReaderGdal::convertLayer(OGRLayer *ogrLayer) const -> std::shared_ptr<GLayer>
{
    ...
    for (;;) {
        ...
        if (OGRGeometry *geometry = ogr_feature->GetGeometryRef()) {
            ...
            try {
                ...
                auto entity = convertEntity(geometry);
                if (!entity) {
                ...
                convertStyles(&ogr_style_mgr, entity.get());

                TableRegister attributes(layer->tableFields());
                convertData(ogr_feature.get(), feature_definition, &attributes);
