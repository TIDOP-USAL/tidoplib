...
#include "homography_lmeds_test.h"
...
HomographyLMedsTest::HomographyLMedsTest()
  : GeometricTest(GeometricTest::Type::homography_lmeds)
{
}

void HomographyLMedsTest::setProperties(const GeometricTest::Properties *properties)
{
    if (const LMedsTestProperties *_properties = static_cast<const LMedsTestProperties *>(properties))
        mProperties = *_properties;
}

auto HomographyLMedsTest::exec(const std::vector<cv::Point2f> &points1, 
