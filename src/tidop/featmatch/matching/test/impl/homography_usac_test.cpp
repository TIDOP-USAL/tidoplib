...
#include "homography_usac_test.h"
...
HomographyUsacTest::HomographyUsacTest()
  : GeometricTest(GeometricTest::Type::homography_usac)
{
}

void HomographyUsacTest::setProperties(const GeometricTest::Properties *properties)
{
    if (const UsacTestProperties *_properties = static_cast<const UsacTestProperties *>(properties))
        mProperties = *_properties;
}

auto HomographyUsacTest::exec(const std::vector<cv::Point2f> &points1,
                              const std::vector<cv::Point2f> &points2) -> std::vector<unsigned char>
{
    ...
    try {
        ...
        cv::findHomography(points1, points2, inliers, params);
