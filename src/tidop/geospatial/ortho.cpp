...
void Orthorectification::run2(const std::vector<experimental::Photo> &photos,
      ...
      for (int r = 0; r < dtm.rows - 1; r++) {
        for (int c = 0; c < dtm.cols - 1; c++) {
          ...
          if (rect_image.contains(photo_image_coordinates[0]) && 
            ...
            if (window_ortho_in.width() <= 0 || window_ortho_in.height() <= 0) continue;
            ...
            if (window_image_in.width() <= 0 || window_image_in.height() <= 0) continue;
