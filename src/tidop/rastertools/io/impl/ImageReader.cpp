...
ImageReader::ImageReader(tl::Path file, Mode mode)
  : mFile(std::move(file))
{
}
...
auto ImageReaderFactory::create(const Path &file, ImageReader::Mode mode) -> ImageReader::Ptr
{
    ...
    try {
        ...
        if (internal::gdalValidExtensions(extension)) {
            image_reader = ImageReaderGdal::New(file, mode);
        } else
#endif
#ifdef TL_HAVE_EDSDK
