...
void ProgressBar::updateProgress()
{
    ...
    std::cout << bar << " " << percent() << "%  completed" << std::flush;
}
...
auto ProgressSpinner::operator()(size_t /*increment*/) -> bool
{
...
void ProgressSpinner::setText(const std::string &message)
{
    std::lock_guard<std::mutex> lock(mMutex);
    mMessage = message;
}

void ProgressSpinner::reset()
{
    std::lock_guard<std::mutex> lock(mMutex);
    mCurrentIndex = 0;
    mMessage.clear();
...
void ProgressSpinner::updateSpinner()
{
    std::cout << "\r" << mSpinnerChars[mCurrentIndex] << " " << mMessage << std::flush;
    mCurrentIndex = (mCurrentIndex + 1) % mSpinnerChars.size();
}
