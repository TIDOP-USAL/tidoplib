 ...
 * \return
 */
 //TL_EXPORT int intersectSegments(const Line &ln1, const Line &ln2, PointI *pt);
 ...
 * \return
 */
//TL_EXPORT int intersectLines(const Line &ln1, const Line &ln2, PointI *pt);
template<typename Point_t>
 ...
 * \param[in] dist
 */
TL_EXPORT void joinLinesByDist(const std::vector<Segment<PointI>> &linesIn, std::vector<Segment<PointI>> *linesOut, int dist);


TL_EXPORT void groupParallelLines(std::vector<Segment<Point<int>>> linesaux, std::vector<GroupLines> *curLinesGrops, double angTol);

TL_EXPORT void groupLinesByDist(const std::vector<Segment<PointI>> &linesIn, std::vector<GroupLines> *curLinesGrops, int dist);

TL_EXPORT void delLinesGroupBySize(std::vector<GroupLines> *vlg, int size);

template<typename Point_t> inline
int pointNearest(const std::vector<Point_t> &pts_fourier, const Point_t &pt_intersect)
{
  double distmin = TL_DOUBLE_MAX;
  int ipt = -1;
...
Point3_t findInscribedCircleSequential(const Polygon3D<Point3_t> &polygon, const BoundingBox<Point3_t> bounds, double xCells, double yCells, double zCells) 
{
  ...
  //max_dist = std::max(max_dist, std::min(increment_x, std::min(increment_y, increment_z)));
	...
	double max_distance = -TL_DOUBLE_MAX;
...
int projectPointInSegment(const Point_t &segment_pt1, const Point_t &segment_pt2, const Point_t &pt, Point_t *ptp)
{
  ...
  if (typeid(typename Point_t::value_type) == typeid(int)) {
    ptp->x = segment_pt1.x + TL_ROUND_TO_INT(v2[0] * r);
    ptp->y = segment_pt1.y + TL_ROUND_TO_INT(v2[1] * r);
  } else {
...
int projectPointInSegment3D(const Point_t &segment_pt1, const Point_t &segment_pt2, const Point_t &pt, Point_t *ptp)
{
  ...
  if (typeid(typename Point_t::value_type) == typeid(int)) {
    ptp->x = segment_pt1.x + TL_ROUND_TO_INT(v2.x * r);
    ptp->y = segment_pt1.y + TL_ROUND_TO_INT(v2.y * r);
    ptp->z = segment_pt1.z + TL_ROUND_TO_INT(v2.z * r);
  } else {
