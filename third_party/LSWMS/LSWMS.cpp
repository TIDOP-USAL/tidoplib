...
void LSWMS::drawLSegs(cv::Mat &img, std::vector<LSEG> &lSegs, std::vector<double> &errors, int thickness)
{
	...
	for(unsigned int i=0; i<lSegs.size(); i++)	
	{
		if(errors[i] < 0.087)  // 5º
            cv::line(img, lSegs[i][0], lSegs[i][1], colors[0], thickness/*3*/);
		else if(errors[i] < 0.174) // 10º
            cv::line(img, lSegs[i][0], lSegs[i][1], colors[1], thickness/*2*/);
		else if(errors[i] < 0.26) // 15º
            cv::line(img, lSegs[i][0], lSegs[i][1], colors[2], thickness/*1*/);
		else 
            cv::line(img, lSegs[i][0], lSegs[i][1], colors[3], thickness/*1*/);
	}
}
