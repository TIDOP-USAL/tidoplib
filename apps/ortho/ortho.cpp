...
int main(int argc, char** argv)
{
  ...
  try {
    ...
    Orthorectification ortho(mdt, crs);
    ortho.run(photos, ortho_path.toString(), footprint_file);
    ...
    //          std::shared_ptr<graph::GPolygon> polygon = bestImage(point, footprint_file);
    //          if (polygon) {
    //            std::shared_ptr<TableRegister> data = polygon->data();
    ...
    //        //  cv::Mat image = image_reader->read(0.05, 0.05, Rect<double>(), &affine);
    ...
    //        //  /// Esquinas
    //        //  corners[i].x = (windows[i].pt1.x - window_all.pt1.x) * 0.05 / scale;
    //        //  corners[i].y = (window_all.pt2.y - windows[i].pt2.y) * 0.05 / scale;
    ...
    //        //cv::Point corner = corners[0] / 0.05;
    //        //cv::Mat gray;
    ...
    //Path orto_final(ortho_path);
    //orto_final.append("orto.tif");
    //std::unique_ptr<ImageWriter> image_writer = ImageWriterFactory::createWriter(orto_final);
    //image_writer->open();
    //int cols = static_cast<int>(std::round(window_all.width() / 0.1));
    //int rows = static_cast<int>(std::round(window_all.height() / 0.1));
    //if (image_writer->isOpen()) {
    //  image_writer->create(rows, cols, 3, DataType::TL_8U);
    //  image_writer->setCRS(crs);
    //  //image_writer->setGeoreference();
    ...
    //      Affine<PointI> affine;
    //      cv::Mat compensate_image = image_reader->read(grid[i],0.1,0.1, &affine);
    //      cv::Mat seam_image = image_reader_seam->read(grid[i], 0.1, 0.1);
    ...
    //        blender->feed(compensate_image_s, seam_image_s, rect.tl());
    //      }
    //    }
    //    cv::Mat _ortho;
    //    cv::Mat _maks;
