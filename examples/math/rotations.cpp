...
//  Eigen::AngleAxisd aa2(1.32, Eigen::Vector3d(3.5, 23.1, 45.3));
//
...
//  q_aa_z = aa_z;
//  Eigen::Quaterniond q_aa_kkkk;
//  q_aa_kkkk = aa2;
//
//  math::AxisAngle<double> axis_angle(1.32, math::Vector3d{ 3.5, 23.1, 45.3 });
//  math::Quaterniond q_rot;
//  math::RotationConverter<double>::convert(axis_angle, q_rot);
//
//
...
//
//
//  Eigen::Quaterniond q_double(2., 0., 1., -3.);
//  ea_xyz = q_double.toRotationMatrix().eulerAngles(0, 1, 2);
//  ea_xzy = q_double.toRotationMatrix().eulerAngles(0, 2, 1);
//  ea_yxz = q_double.toRotationMatrix().eulerAngles(1, 0, 2);
//  ea_yzx = q_double.toRotationMatrix().eulerAngles(1, 2, 0);
//  ea_zxy = q_double.toRotationMatrix().eulerAngles(2, 0, 1);
//  ea_zyx = q_double.toRotationMatrix().eulerAngles(2, 1, 0);
//  ea_zxz = q_double.toRotationMatrix().eulerAngles(2, 0, 2);
//  ea_xyx = q_double.toRotationMatrix().eulerAngles(0, 1, 0);
//  ea_yzy = q_double.toRotationMatrix().eulerAngles(1, 2, 1);
//  ea_zyz = q_double.toRotationMatrix().eulerAngles(2, 1, 2);
//  ea_xzx = q_double.toRotationMatrix().eulerAngles(0, 2, 0);
//  ea_yxy = q_double.toRotationMatrix().eulerAngles(1, 0, 1);

  Eigen::Matrix3d m;
  m << /*-0.8888889, 0.4444444, -0.1111111,
        -0.1111111, -0.4444444, -0.8888889,
        -0.4444444, -0.7777778, 0.4444444;*/
  ...
  Eigen::Vector3d ea_xyz = m.eulerAngles(0, 1, 2);
  Eigen::Vector3d ea_xzy = m.eulerAngles(0, 2, 1);
  Eigen::Vector3d ea_yxz = m.eulerAngles(1, 0, 2);
  Eigen::Vector3d ea_yzx = m.eulerAngles(1, 2, 0);
  Eigen::Vector3d ea_zxy = m.eulerAngles(2, 0, 1);
  Eigen::Vector3d ea_zyx = m.eulerAngles(2, 1, 0);
  Eigen::Vector3d ea_zxz = m.eulerAngles(2, 0, 2);
  Eigen::Vector3d ea_xyx = m.eulerAngles(0, 1, 0);
  Eigen::Vector3d ea_yzy = m.eulerAngles(1, 2, 1);
  Eigen::Vector3d ea_zyz = m.eulerAngles(2, 1, 2);
  Eigen::Vector3d ea_xzx = m.eulerAngles(0, 2, 0);
  Eigen::Vector3d ea_yxy = m.eulerAngles(1, 0, 1);

//  Eigen::Matrix3d m_ones = Eigen::Matrix3d::Ones();
//  Eigen::Matrix3d m_identity = Eigen::Matrix3d::Identity();

  Quaterniond q;
  q = Eigen::AngleAxisd(2.356194490192345, Eigen::Vector3d::UnitY()) *
      Eigen::AngleAxisd(2.5261129449194057, Eigen::Vector3d::UnitZ()) *
      Eigen::AngleAxisd(0.7853981633974483, Eigen::Vector3d::UnitX());

  Eigen::AngleAxisd angle_axis;
  angle_axis = q;
  angle_axis.axis();

  std::cout << "AngleAxisd:" << std::endl;
  std::cout << "Angle: " << angle_axis.angle() << " - " << angle_axis.axis() << std::endl;
