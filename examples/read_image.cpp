...
int main(int argc, char** argv)
{
  ...
  std::vector<std::vector<TrfPerspective<PointD>>> trfs(6, std::vector<TrfPerspective<PointD>>(6));
  std::vector<std::vector<std::vector<match_hugin>>> points(6, std::vector<std::vector<match_hugin>>(6));
    
  std::vector<std::vector<TrfPerspective<PointD>>> trfs_tm(6, std::vector<TrfPerspective<PointD>>(6));
  std::vector<std::vector<std::vector<match_hugin>>> points_template_matching(6, std::vector<std::vector<match_hugin>>(6));
  ...
  for (size_t i = 0; i < trfs.size(); ++i) {
    for (size_t j = 0; j < trfs[0].size(); ++j) {
      ...
      std::vector<PointD> points1(n);
      std::vector<PointD> points2(n);
      ...
      RasterGraphics img1;
      RasterGraphics img2;
      TL::RasterGraphics::Status err1 = img1.open("C:\\Users\\Esteban\\Documents\\Digiflights\\Projects\\Zeiss_grid\\20180314\\corregidas\\1111\\TEST070_INTER_CUBIC.tif", TL::File::Mode::Read);
      TL::RasterGraphics::Status err2 = img2.open("C:\\Users\\Esteban\\Documents\\Digiflights\\Projects\\Zeiss_grid\\20180314\\corregidas\\1111\\TEST071_INTER_CUBIC.tif", TL::File::Mode::Read);
      int channels = img1.getBands();
      WindowD w1(PointD(), PointD(img1.getCols(), img1.getRows()));
      WindowD w2(w1);
      WindowD w2_t1;
      TL::transform(w2, &w2_t1, &(trfs[i][j]), transform_order::INVERSE);
      WindowD w_inter = TL::geometry::windowIntersection(w1, w2_t1);

      //TL::transform(w3, &w2_, &(trfs[i][j]));
      ...
      if (width > height) {
        ...
        for (int i_r = 0; i_r < r; i_r++) {
          for (int i_c = 0; i_c < c; i_c++) {
            ...
            if (channels == 2) {
              ...
              cv::Mat mat_split[2];
              cv::split(img_templ, mat_split);
              img_templ = mat_split[0].clone();
            } else if (channels >= 3) {
        ...
        size_t n_tm = points_template_matching[i][j].size();

        std::vector<PointD> points1_tm(n_tm);
        std::vector<PointD> points2_tm(n_tm);
        for (size_t ip = 0; ip < points_template_matching[i][j].size(); ip++) {
          points1_tm[ip] = points_template_matching[i][j][ip].pt1;
          points2_tm[ip] = points_template_matching[i][j][ip].pt2;
        }

        std::vector<double> error_tm;
        double rmse_tm = 0.0;
        trfs_tm[i][j].compute(points1_tm, points2_tm, &error_tm, &rmse_tm);
        ...
        while (rmse_tm > 0.25) {
          ...
          if (points1_tm.empty() || points1_tm.size() < 5 || iter == 10) break;

          if (n_pts == points1_tm.size()) {
            // No se ha eliminado ningun punto
            mult_rmse -= 0.25;
          } else {
            std::vector<double> error2;
            double rmse2;
            TrfPerspective<PointD> trf;
            trf.compute(points1_tm, points2_tm, &error2, &rmse2);
            if (rmse2 < rmse_tm) {
              rmse_tm = rmse2;
              trfs_tm[i][j] = trf;
              //points[i][j].resize(points1_tm.size());
              //for (size_t k = 0; k < points1_tm.size(); k++) {
              //  points[i][j][k].pt1 = points1_tm[k];
              //  points[i][j][k].pt2 = points2_tm[k];
              //}
            }
          }
          iter++;
