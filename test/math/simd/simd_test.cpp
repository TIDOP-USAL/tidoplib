...
BOOST_FIXTURE_TEST_CASE(matrix_mul_double_3_2, PackedTest)
{
...
BOOST_FIXTURE_TEST_CASE(matrix_mul_double_3_3, PackedTest)
{

  Matrix<double, 10, 10> mat = Matrix<double, 10, 10>::zero();

  size_t max_vector = (matrix2.cols() / (packed_b_d.size() * 2)) * packed_b_d.size() * 2;
  Packed<double> packed_c;
  ...
  for (size_t r = 0; r < iter; r += 8) {
    for (size_t i = 0; i < matrix1.cols(); i++) {
      ...
      for (size_t c = 0; c < max_vector; c += step*2) {

        packed_b_d.loadUnaligned(&matrix2[i][c]);
        packed_b2.loadUnaligned(&matrix2[i][c + step]);
        ...
        packed_c.loadUnaligned(&mat[r][c + step]);
        packed_c += packed_a1 * packed_b2;
        packed_c.storeUnaligned(&mat[r][c + step]);

        packed_c.loadUnaligned(&mat[r + 1][c + step]);
        packed_c += packed_a2 * packed_b2;
        packed_c.storeUnaligned(&mat[r + 1][c + step]);

        packed_c.loadUnaligned(&mat[r + 2][c + step]);
        packed_c += packed_a3 * packed_b2;
        packed_c.storeUnaligned(&mat[r + 2][c + step]);

        packed_c.loadUnaligned(&mat[r + 3][c + step]);
        packed_c += packed_a4 * packed_b2;
        packed_c.storeUnaligned(&mat[r + 3][c + step]);

        packed_c.loadUnaligned(&mat[r + 4][c + step]);
        packed_c += packed_a5 * packed_b2;
        packed_c.storeUnaligned(&mat[r + 4][c + step]);

        packed_c.loadUnaligned(&mat[r + 5][c + step]);
        packed_c += packed_a6 * packed_b2;
        packed_c.storeUnaligned(&mat[r + 5][c + step]);

        packed_c.loadUnaligned(&mat[r + 6][c + step]);
        packed_c += packed_a7 * packed_b2;
        packed_c.storeUnaligned(&mat[r + 6][c + step]);

        packed_c.loadUnaligned(&mat[r + 7][c + step]);
        packed_c += packed_a8 * packed_b2;
        packed_c.storeUnaligned(&mat[r + 7][c + step]);
