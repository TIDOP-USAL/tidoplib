...
BOOST_FIXTURE_TEST_CASE(Within_Point_Point, WithinTestFixture)
{
    BOOST_CHECK(within(p1, p1));                     // mismo punto
    BOOST_CHECK(!within(p1, p2));                    // distintos

    // Con tolerancia
    Point2d p1_almost(1e-9, 1e-9);                   // muy cerca de p1 (p1 es 0,0)
    BOOST_CHECK(within(p1_almost, p1, 1e-8));        // tolerancia mayor que distancia
    BOOST_CHECK(!within(p1_almost, p1, 1e-10));      // tolerancia menor
}
...
BOOST_FIXTURE_TEST_CASE(Within_Tolerance, WithinTestFixture)
{
    ...
    Point2d almost_on_line(0.5, 0.5000001);
    BOOST_CHECK(within(almost_on_line, line_diagonal, 1e-6));
    BOOST_CHECK(!within(almost_on_line, line_diagonal, 1e-10));

    // Segmento casi contenido
    Segment<Point2d> almost_inside(Point2d(0.0, 1e-8), Point2d(1.0, 1.0 - 1e-8));
    BOOST_CHECK(within(almost_inside, square, 1e-7));
    BOOST_CHECK(!within(almost_inside, square, 1e-9));
}
