...
BOOST_FIXTURE_TEST_CASE(origin, RectTest)
{
    BOOST_CHECK_EQUAL(0, rect_default_constructor_integer.origin().x());
    BOOST_CHECK_EQUAL(0, rect_default_constructor_integer.origin().y());

    BOOST_CHECK_EQUAL(5, rect_integer->origin().x());
    BOOST_CHECK_EQUAL(10, rect_integer->origin().y());
    BOOST_CHECK_CLOSE(5.5, rect_double->origin().x(), 0.01);
    BOOST_CHECK_CLOSE(10.6, rect_double->origin().y(), 0.01);

    BOOST_CHECK_CLOSE(5.5f, rect_float->origin().x(), 0.01);
    BOOST_CHECK_CLOSE(10.6f, rect_float->origin().y(), 0.01);
}
