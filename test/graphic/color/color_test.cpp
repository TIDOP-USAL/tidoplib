...
struct ColorTest
{
    ...
    void setup()
    {
        color_name = Color(Color::Name::azure);
        color_string = Color("40e0d0" /*turquoise*/);
    }
...
{
    BOOST_CHECK_EQUAL(0, color1.rgb());
    BOOST_CHECK_EQUAL(4278190080, color1.argb());
    BOOST_CHECK_EQUAL(0, color1.red());
    BOOST_CHECK_EQUAL(0, color1.green());
    BOOST_CHECK_EQUAL(0, color1.blue());
    BOOST_CHECK_EQUAL(255, color1.alpha());
    BOOST_CHECK_EQUAL(0, color1.luminance());
    BOOST_CHECK_EQUAL("000000", color1.toHexRGB());
    BOOST_CHECK_EQUAL("FF000000", color1.toHexARGB());
    BOOST_CHECK_EQUAL("000000FF", color1.toHexRGBA());
}

BOOST_FIXTURE_TEST_CASE(name_constructor, ColorTest)
{
    BOOST_CHECK_EQUAL(15794175, color_name.rgb());
    BOOST_CHECK_EQUAL(240, color_name.red());
    BOOST_CHECK_EQUAL(255, color_name.green());
    BOOST_CHECK_EQUAL(255, color_name.blue());
    BOOST_CHECK_EQUAL(255, color_name.alpha());
    BOOST_CHECK_EQUAL(252, color_name.luminance());
    BOOST_CHECK_EQUAL("F0FFFF", color_name.toHexRGB());
    BOOST_CHECK_EQUAL("FFF0FFFF", color_name.toHexARGB());
    BOOST_CHECK_EQUAL("F0FFFFFF", color_name.toHexRGBA());

}

BOOST_FIXTURE_TEST_CASE(string_constructor, ColorTest)
{
    BOOST_CHECK_EQUAL(4251856, color_string.rgb());
    BOOST_CHECK_EQUAL(64, color_string.red());
    BOOST_CHECK_EQUAL(224, color_string.green());
    BOOST_CHECK_EQUAL(208, color_string.blue());
    BOOST_CHECK_EQUAL(255, color_string.alpha());
    BOOST_CHECK_EQUAL(189, color_string.luminance());
    BOOST_CHECK_EQUAL("40E0D0", color_string.toHexRGB());
    BOOST_CHECK_EQUAL("FF40E0D0", color_string.toHexARGB());
    BOOST_CHECK_EQUAL("40E0D0FF", color_string.toHexRGBA());
}

BOOST_FIXTURE_TEST_CASE(set_alpha, ColorTest)
{
    Color azure(Color::Name::azure);
    azure.setAlpha(128);
    BOOST_CHECK_EQUAL(128, azure.alpha());
}
...
BOOST_FIXTURE_TEST_CASE(color_from_color_rgb, ColorTest)
{
    ...
    Color color(rgb);
    BOOST_CHECK_EQUAL(661790, color.rgb());
}
...
BOOST_FIXTURE_TEST_CASE(color_from_color_cmyk, ColorTest)
{
    ColorCMYK cmyk(1., 0.2510, 0., 0.);
    Color color(cmyk);
    auto red = color.red();
    auto green = color.green();
    auto blue = color.blue();
    auto alpha = color.alpha();
    BOOST_CHECK_EQUAL(49151, color.rgb());
}

BOOST_FIXTURE_TEST_CASE(color_from_color_hsv, ColorTest)
{
    ColorHSV color_hsv(240., 100., 100.);
    Color color(color_hsv);
    BOOST_CHECK_EQUAL(0x0000FF, color.rgb());
}

BOOST_FIXTURE_TEST_CASE(color_from_color_hsl, ColorTest)
{
    ColorHSL color_hsl(240., 100., 25.);
    Color color(color_hsl);
    BOOST_CHECK_EQUAL(0x000080, color.rgb());
}
...
BOOST_AUTO_TEST_CASE(ColorRGB_toColor)
{
    ...
    BOOST_CHECK_EQUAL(color.red(), 255);
...
BOOST_AUTO_TEST_CASE(ColorRGB_fromColor)
{
    ...
    ColorRGB rgb = ColorRGB::fromColor(color);
    BOOST_CHECK_EQUAL(0, rgb.red());
...
BOOST_FIXTURE_TEST_CASE(default_constructor, ColorRGBATest)
{
    ...
    BOOST_CHECK_EQUAL(255, rgba_def.alpha());
...
{
    Color color(Color::Name::deep_sky_blue);
    ColorRGBA rgba= ColorRGBA::fromColor(color);
    BOOST_CHECK_EQUAL(0, rgba.red());
...
BOOST_FIXTURE_TEST_CASE(constructor, ColorCMYKTest)
{
    ...
    BOOST_CHECK_EQUAL(255, color.red());
    BOOST_CHECK_EQUAL(0, color.green());
    BOOST_CHECK_EQUAL(0, color.blue());
    BOOST_CHECK_EQUAL(255, color.alpha());
}
...
BOOST_AUTO_TEST_CASE(ColorCMYK_toColor)
{
    ...
    Color color = color_cmyk.toColor();
    BOOST_CHECK_EQUAL(color.red(), 0);
    BOOST_CHECK_EQUAL(color.green(), 100);
    BOOST_CHECK_EQUAL(color.blue(), 0);
    BOOST_CHECK_EQUAL(color.alpha(), 255);
}
...
{
    Color color(Color::Name::deep_sky_blue);
    ColorCMYK color_cmyk = ColorCMYK::fromColor(color);
...
struct ColorHSVTest
{
    ...
    void setup()
    {
        hsv = ColorHSV(259.592, 57.647, 33.333);
    }
...
BOOST_AUTO_TEST_CASE(ColorHSV_toColor)
{
    ...
    Color color = color_hsv.toColor();
    BOOST_CHECK_EQUAL(color.red(), 243);
    BOOST_CHECK_EQUAL(color.green(), 55);
    BOOST_CHECK_EQUAL(color.blue(), 123);
    BOOST_CHECK_EQUAL(color.alpha(), 255);
}

BOOST_AUTO_TEST_CASE(ColorHSV_fromColor)
{
    Color color(243, 55, 123);
    ColorHSV color_hsv = ColorHSV::fromColor(color);
...
BOOST_AUTO_TEST_CASE(ColorHSL_fromColor)
{
    Color color(243, 55, 123);
    ColorHSL color_hsl = ColorHSL::fromColor(color);
    BOOST_CHECK_CLOSE(338.298, color_hsl.hue(), 0.1);
    BOOST_CHECK_CLOSE(88.679, color_hsl.saturation(), 0.1);
    BOOST_CHECK_CLOSE(58.431, color_hsl.lightness(), 0.1);
}
